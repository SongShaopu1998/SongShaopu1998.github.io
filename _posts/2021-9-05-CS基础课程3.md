...
二分查找要分为整数二分查找和浮点数二分查找。其中整数查找因为要解决**各自的边界问题(中间的分割点有`mid`, `mid + 1`)**，所以更为复杂，存在两种情况，这里我们会用到一个`check`函数，帮助检查此时的`mid`满足**1**部分的性质还是满足**2**部分的性质，如果我们想要检测**2部分的分界**，那么要怕段`mid`是否满足**2(即右侧)**部分具备的性质：
...
而如果我们想要检测的是`mid`是否满足**1(即左侧)**的性质:
