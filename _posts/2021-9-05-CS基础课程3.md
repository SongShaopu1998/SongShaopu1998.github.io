...
```c++
void minHeapify(PriorityQueue*& heap, int i) {
    // we need to find the smallest node among the current node
    // and its children
    int left = i * 2;
    int right = 2 * i + 1;
    int smallest = 0;
    ...
    if (right <= size && heap[right].priority <= heap[i].priority) {
        smallest = right;
    }
    if (smallest != i) {
...
图的实现方法有很多，在UIUC的课程中，主要提到了这三种实现方法：

- Edge List

<img src="https://shaopu-blog.oss-cn-beijing.aliyuncs.com/img/202110252154636.png" alt="image-20211025215458576" style="zoom: 50%;" />

这是一种基础的图表示方式，其中顶点的集合是一个数组或者一个哈希表；在存储边的信息时，我们将由边所连接的顶点包含进来，并构成一个链表结构，每一个位置存储的是`<v, v, key>`的元组。

- Adjacency Matrix

<img src="https://shaopu-blog.oss-cn-beijing.aliyuncs.com/img/202110252156225.png" alt="image-20211025215649149" style="zoom: 50%;" />
...
<img src="https://shaopu-blog.oss-cn-beijing.aliyuncs.com/img/202110252157376.png" alt="image-20211025215704303" style="zoom: 50%;" />

这种结构其实类似于我们在中文课本上学过的邻接多重表，如果只保留它的左半部分，那么这将是一个邻接表。

邻接表简而言之，就是在顶点表中的每一个顶点后，都跟上一组`neighbors`，代表了所有对于该顶点的`indicent edges`。
...
与BFS不同的是，这里我们使用的是**优先级队列**，且注意：

1. 不能在找到要求节点时停止，这也是与BFS不同的一点，因为如果在找到节点时立刻停下，找到的路径未必是代价最小的，只有当此节点从队列中出列时，也即队列为空时，我们才会停止搜索过程。
...
具体来说，**对于每一个根节点，也即每一个独立子集的表示元素，在并查集中的存储值小于0**，具体存储的值因为**smart union**的存在，为**节点（元素）的个数**。

> 如果采取另一种优化策略，即储存$-(h+1)$，那么其效率将不如储存节点数高。因为当我们建好没有经过路径压缩的*upTree*后，查找操作的时间复杂度为$O(lgn)$，那么将节点数目少的树合并到节点数目多的树上是一种更划算的操作。

**对于所有的非根节点**，我们全部在对应的位置上存储其根节点的**index**，相当于一个指针，我们就可以顺着找到根节点的位置了。**这样一来，如果两个位置上存储了同一个正数，也即数组中的某一个位置，那么此两节点必定位于同一个集合中，因为它们具有一样的表示元素**。

为了进一步减小时间复杂度，是用了路径压缩的技巧，这是因为我们是否可以不用一直沿着树的节点向上找到根节点，再判定查找结果？如果能够把树的子树全部展开，那么效果其实是一样的，但是我们只需要网上找一层父节点了。
