...
2. 此类问题以及后边即将出现的区间更新，均被分为三种情况予以讨论：no overlap, total overlap, partial overlap，分别对应了代码中的base case以及recursive case。**但要注意的是，这里的重合情况的对象是由给定区间指向总长度的，而非查看总长度相对于给定区间的重合情况。**
...
void SegmentTree::updateSegmentTreeRangeHelper(int low, int high, int qlow, int qhigh, int pos, int delta) {
    // put the low>high case and the "no overlap" case together
