...
但如果不是`full cone NAT`，情况就变得更加复杂：在完成了上述步骤之后，A与B可以通过同时向NAT发送想要与B或者A通信的packet，来向NAT中添加对应的mapping，当packet越过NAT到达接收端时，就可以通过刚添加的mapping信息成功建立通信。
...
最后，resolver会将结果缓存起来并转发给client，如果下次再有别的client需要向resolver查询同一个DNS query，他可以直接使用自己的caching。
