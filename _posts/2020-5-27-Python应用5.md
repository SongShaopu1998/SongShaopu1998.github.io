...
其中，位置参数就是最一般的根据所在位置决定的参数，在函数的调用中，具体那个参数属于哪一类，这是要取决于前后的参数和自己所在的位置的，比如，如果函数定义中只有一个位置参数，紧跟着就是包裹位置参数，那调用的时候第二个自然就属于包裹位置参数，如果一个位置参数和默认参数都没有，那一上来就是包裹位置参数。
...
这种参数表示法的好处就在于我们可以将所有的，不论什么形式的`foo`的参数都可以完整的传递到`func()`中去.
...
因为内层的函数始终能记住外部的嵌套作用域，所以这就无限套娃了。
...
同理，首先执行`log('execute')`，返回的是`decorator`函数，再调用返回的函数，参数是`now`函数，返回值是`wrapper`函数，此时传给`now`的参数会传给`wrapper`，再调用`wrapper`时把接收到的参数传给其中的`now`函数，最后返回的是`now`调用的值。
...
装饰器的基本内容就学习到这儿，因为我对于Python的类方法还不是很熟悉，所以接下来学习Python的类。