...
同理，首先执行`log('execute')`，返回的是`decorator`函数，再调用返回的函数，参数是`now`函数，返回值是`wrapper`函数，执行`now = log('execute')(now)`后，变量名`now`现在与`wrapper`函数绑定. 原本`now`函数中的参数现在被传递给`wrapper`函数，再调用`wrapper`时把接收到的参数传给其中的`now`函数，最后返回的是`now`调用的值。
