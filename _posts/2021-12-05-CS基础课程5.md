...
结合以上描述，我们知道这一段中断的过程对于程序本身其实是不易被察觉的，当使用**ECALL**时，类似于进行了一个`function call`，在其他情况下，就好比无事发生。但是另一方面，**cache**可能会变成垃圾值，因为有别的任务在使用内存。

> In fact, for security reasons the **OS** often needs to **flush all caches** before returning flow to the program

上述规则表明，在每一次`context switch`之后我们都会在**cache**中得到大量的`compulsory miss`.
