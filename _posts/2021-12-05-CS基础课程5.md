...
显然，许多程序，比如我们先前所使用的**Venus**都尝试使用所谓的“同一片内存区域”，为了解决这个问题，操作系统会进行**virtual address**与**physical address**的转换。这样即使对于程序本身来说，看起来是使用了同一块内存，实际上那只是`virtual address`而已，操作系统为他们分配到了不同的`physical address`.
