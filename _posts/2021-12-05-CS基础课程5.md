...
对于**L1 cache**，当`array_size`为`32kb`时，没有出现任何**miss**，说明此时还未达到`cache size`. 而在`64kb`时出现了多次**miss**，故`cache size`为`32kb`.（一般来讲，`cache size`都是2的指数）

而在`64kb`的测试内部我们观察到，当`stride`在`128bytes`之前没有出现**miss**，
