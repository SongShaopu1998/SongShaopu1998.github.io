...
进一步分析，是CPU的**Control Unit**（控制器）读取指令，控制CPU中**Datapath**（运算器）里的寄存器们，并由CPU内部的**PC(Program Counter)**寄存器记录**下一条需要执行的指令的`byte address`**（update PC）实现的：
...
在函数调用之前，我们先要将被储存在`saved register`中的两个变量a, b放入用于储存函数参数的寄存器**a0**与**a1**中。（如果此时这几个寄存器的容量不足以存储函数参数，那么需要借助`stack`，这个后边会提到）。
...
#### Nested Calss & Register Conventions
...
可以看到，在函数最开始我们调整栈顶指针的位置，开辟新的栈空间，同时将寄存器**s0**,**s1**存入刚开辟的栈空间内。在程序结束之后，恢复寄存器的内容，并释放栈空间。

那么问题来了，如果我们面对的是**嵌套函数调用**，怎么办？哪些寄存器里的内容应当暂时被放到栈内保存？
