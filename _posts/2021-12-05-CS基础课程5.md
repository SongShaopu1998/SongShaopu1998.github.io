...
这种情况我们之后会详细介绍，处理器会检测“状态”是否有变化，如果检测到有**事件**发生，则会通过**异常表(`exception table`)**进行一个间接过程调用(异常)，到一个专门设计用来处理这类事件的操作系统子程序，即**异常处理程序(`exception handler`)**。**在这门课中**，我们称之为执行`Trap handler`(被操作系统以`supervisor mode`调用)，如上图所示。
