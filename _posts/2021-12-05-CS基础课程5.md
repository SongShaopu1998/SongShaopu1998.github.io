...
操作系统将应用对于系统资源的使用施加了一些限制，比如对于内存以及设备等，在`user mode`下我们不能直接访问**内核代码和数据**，只能通过`syscall`接口来访问。如此一来即使应用崩溃了也不会对系统整体造成影响。
...
调用**syscalls**的方法也很简单，正如先前在**Venus**中使用的**ECALL**指令那样，我们通过给寄存器一些特殊的指定的`bits`来告诉操作系统执行哪一个对应的**syscalls**，而操作系统接下来会通过调用专门生成`software interrrupt`的指令来实现操作（**ECALL**）. 实际上，`software interrupt`并非属于`interrupt`的异常类型，它是由运行的程序本身产生的，**syscall**在《深入理解计算机系统》中属于`trap`类型。
...
**需要注意的是，这里的异常定义与《深入理解计算机系统》中的定义不完全一样，这里的`exception`对应着书中的`trap+fault+abort`，被称为故障指令(`faulting instruction`)。之后介绍的`page fault`就是一种`fault`异常. `interrupt`对于程序本身是`异步`的，而其他三种都是`同步`的。**
...
而正是由于这些原因，我们得以通过利用虚拟内存对物理内存建立一种映射：
...
也正是基于“虚拟内存将主存视作磁盘上的地址空间的**cache**”这一想法，我们对地址提出`paged memory`的想法：

![image-20220131143025474](https://shaopu-blog.oss-cn-beijing.aliyuncs.com/img/image-20220131143025474.png)

**每一个进程都有一个独立的`page table`**.地址格式中的`offset`告诉我们在主存/磁盘中的`page memory`中具体的地址位置；而前边的`page number`则帮助我们确定`page table`中的**对应索引位置**：
...
根据先前对`paged memory`三种状态的描述：`page fault`应当考虑两种情况：

- 未缓存
- 未分配

![image-20220131155516401](https://shaopu-blog.oss-cn-beijing.aliyuncs.com/img/image-20220131155516401.png)

首先检查是否**分配**，如果没有，则在**DRAM**中分配一个新的页；之后检查是否**缓存**，如果没有，也作出相应处理，过程如下所示：

![image-20220131155658701](https://shaopu-blog.oss-cn-beijing.aliyuncs.com/img/image-20220131155658701.png)

这个过程中，我们可能需要选择一个**牺牲页**(`page to evict`).
...
如果我们需要将页从磁盘中加载，那么可能需要进行`context switch`来完成此任务。最终，命令需要重新执行。
...
1. 增大`page size`，这样一来**PN**就会减小，但是同时带来一个问题，即太大的`page`意味着其中很多内存块都没有被使用。。。但是一直占着坑。
...
为了理解这句话，我们将各级页表想象为一个**树结构**。注意到顶级页表只能有一张，因为如果有两个顶级页表，由于在每一张`page table`中`page number`都是从**0**开始计数，那么就无法区分它们。而增加额外的索引区分机制势必增加更多的结构复杂性。如此一来，根据**递归**的思想，在每一层的每一个分支的`page table`最大只能是一个`page`的大小。(这里各级页表的大小指的是*PPN1, PPN2,…*)
...
但是问题是，**Address translation**是十分昂贵的！即使是在借助**cache**的帮助下：
...
而如果我们真的需要从**DRAM**中取出对应的`page table`信息，那么可能会耗费上百个时钟周期。

于是我们想是否可以借助于**cache**的思想？避免还需要从**cache**中取出地址（其实实际上，**TLB**的提出要早于**cache**）
...
**TLB**通常具有32-128个`entry`，并被设计成`fully-associative`。有时在较大的较为复杂的**TLB**中，我们会使用`4-8 way set-associative`来做权衡。
...
当我们生成一个子进程时，仅复制一份`page table`和对应的状态寄存器内容，并将`page table`的原本和副本都设置为**read-only**，只有当我们尝试向某个进程中写入时，因为此时想要写入**read-only memory**，就会在**protection check**时进入`protection fault handler`，由于知道了具体需要写入的`page`，就可以复制一份`page`：

![image-20220201194923638](https://shaopu-blog.oss-cn-beijing.aliyuncs.com/img/image-20220201194923638.png)

> 这里的**Process P**与**Process Q**为`page table`.

并将这两个`page table`中对应着`PPN2`设置为**可写**.以后，如果我们还要更新**Process P**的`page 3`的内容就可以直接更新了。
...
1. 文件（要经过磁盘，很慢）
2. pipe
3. shared memory–**相同的物理内存可以对应着不同的虚拟内存**

##### Memory Mapped I/O

上一节提到，一直从磁盘中读写文件是很慢的，因为每次读取一行，都要跑到磁盘中读取一下。所以我们是否可以借助缓存的思想，每一次转移一大块文件内容到内存中，减少前往磁盘的次数？

当然可以的，`virtual memory`就可以用于实现这个目的——因为它一次将一个`page`的内容搞到内存中，这也是我们使用Memory mapped I/O 带来的一个潜在益处 ：
...
![image-20220202171716905](https://shaopu-blog.oss-cn-beijing.aliyuncs.com/img/image-20220202171716905.png)

观察上边这个表，可以总结出4个输入/输出方面的挑战：

- Extremely wide range of data rates
- Differing physical interfaces(electrical signal, timing)
- Device specific semantics (initialization, control, data-movement)
- Unknown future devices

相应地，我们可以采取如下的解决方案：

1. ![image-20220202172220282](https://shaopu-blog.oss-cn-beijing.aliyuncs.com/img/image-20220202172220282.png)

我们为**CPU**提供一个看待外设的统一视角：从硬件连接上来说，提供了**PCI**、**USB**、**SATA**、**ETHERNAT**等不同的协议类型；从在软件层面也提供统一的标准编程模型。

> - **device abstracted as files**是在**Unix**系统诞生时被提出的思想，即万物皆为文件.
> - **OS input/output library**为一些内核函数(read/write)
> - **Language specific I/O library**为`printf()`这一类的函数
