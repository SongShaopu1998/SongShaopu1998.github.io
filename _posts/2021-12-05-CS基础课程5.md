...
![image-20220131172029616](https://shaopu-blog.oss-cn-beijing.aliyuncs.com/img/image-20220131172029616.png)
...
对于**TLB**的替换策略，一般使用的是**FIFO**，因为对于我们目标的`one clock cycle`而言，**LRU**策略的时间成本有点高。
...
处理**TLB miss**的方法通常是硬件处理；而**page fault**需要`precise trap`，故我们可以通过使用`software handler`在缓存对应的`page`后恢复状态。
...
通过计算“需要从磁盘中获取数据”这一情况的**AMAT**，我们发现$$HR_{Mem}$$必须很贴近1，才能够**AMAT**保证达到可以接受的程度。
