...
- 此外，我们还要注意，在实际编程中，我们可能经常需要向一个函数传递数组，此时我们必须要向函数传递一个额外的`size`参数，否则仅通过一个数组指针，函数无法得知数组具体的大小！
- 在**C**中使用字符串（字符数组）时，末尾的`\0`在**ASCII**码中对应了**NUL**，即**ASCII**码中的**0**！所以我们在判定字符串是否到达末尾时，可以直接将`*str++*`作为判定条件，而无需和`\0`比较。
