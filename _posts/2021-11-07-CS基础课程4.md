...
特别的是，在*Scott Meyers*的文章中，有这么一个例子：
...
什么是`perfect forwarding`?通俗的讲，就是如果我们通过一个函数将一个参数转发给另一个函数处理，在传递的过程中，参数始终能够保持先前的特征，比如右值始终未为右值，左值始终为左值。[这里](https://stackoverflow.com/questions/3582001/what-are-the-main-purposes-of-using-stdforward-and-which-problems-it-solves)详细讨论了这一主题。
...
那么最终的结果我们希望与`factory function`不存在的情况一样，
...
为什么这种方式可以实现完美转发？我们可以尝试向上述代码中分别传递左值与右值以[验证结果](http://thbecker.net/articles/rvalue_references/section_08.html#footnote_1)。
