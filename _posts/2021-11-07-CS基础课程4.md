...
- 单向（`single-pass`），迭代器对应的每个位置只能允许被经过一次
...
这种迭代器的应用场景有之前提到过的`find`以及`count`函数等。它是**只读的**。
...
承接上一种类型，但是支持`+-`与`-=`操作符，同时可以使用`offset dereference operator ([])`，应用在容器`vector`, `deque`, `string`以及**指针**上。
