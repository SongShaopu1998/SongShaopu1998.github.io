...
这意味着，即使拥有相同的函数签名，我们也不能对`lambda`做赋值行为：

```c++
auto f = [](int a, int b){ return a + b; };
f = [](int a, int b){ return a + b + 1; };  // oop!
```

不过当**捕获列表为空**时，我们可以将`lambda`表达式赋值给一个拥有相同函数签名的函数指针，**相应的函数指针指向的是lambda表达式内部的一个static成员函数**：
...
而在**C++11**中，提出了`std::function`，我们也可以利用`std::function`来把`lambda`表达式传入函数中：
