...
> 在这里，`iterator_traits`是一个`meta-function`，返回的是根据给入参数决定的迭代器的`type`，也即上图中的各种`iterator_tag`，我们可以利用这些类别构造我们自己的函数，在下方模板一章节的最后一个例子中有提到（来自于*Stanford CS 106L Fa2020 lecture*）。
...
如果说我们传入的是左值，那么程序仍然会按照之前的重载函数运行（包含复制等一系列操作），我们不能偷取左值中存放的内容，因为之后可能还会用到；但如果我们传入的是右值，因为右值表达式行将销毁，基于这一特性，我们想**把等式右边的内容偷过来，或者说我们想让等式左边的内容的指针指向等式右边的右值的内容，而不是把右值的内容复制到左边一份**。这种操作（想法）被叫做`move semantics`，于是我们有了一个期望的函数模板：
...
程序对于这两种成员重载函数的选择通过`overlaod resolution`实现：
...
**问题来了，为什么转化成一个右值引用的`type`我们就可以转移对象控制权了呢？**
...
<img src='https://shaopu-blog.oss-cn-beijing.aliyuncs.com/img/IMG_0219.PNG' alt='IMG_0219' style='zoom:50%'/>

只要我们在类中包含了虚函数，在哪存的对象模型中就会有且始终仅有一个**虚指针**（`virtual pointer`），而观察虚指针所在的内存可以看到，**派生类中包含有每个基类的子对象，也即派生类中含有基类的部分！**。这个虚指针指向**虚表**（`virtual table`），虚表中存放的是所有指向该类对象内部虚函数的指针。当我们使用指针`p`去调用类对象时，动态绑定决定了`p`指针指向的位置（指向哪一个类对象的内存位置），那这个`p`是什么呢？

<img src='https://shaopu-blog.oss-cn-beijing.aliyuncs.com/img/IMG_0221.PNG' alt='IMG_0221' style='zoom:50%'/>
