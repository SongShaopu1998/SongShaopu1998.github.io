...
<img src="https://shaopu-blog.oss-cn-beijing.aliyuncs.com/img/202110312240802.png" alt="image-202110312240802" style="zoom:5%;" />
...
https://shaopu-blog.oss-cn-beijing.aliyuncs.com/img/IMG_0219.PNG

只要我们在类中包含了虚函数，在哪存的对象模型中就会有且始终仅有一个**虚指针**（`virtual pointer`），而观察虚指针所在的内存可以看到，**派生类中包含有每个基类的子对象，也即派生类中含有基类的部分！**。这个虚指针指向**虚表**（`virtual table`），虚表中
