...
既然提到了`concepts`关键字，那么接下来就针对该关键字进行分析。

### About concepts

接下来所述的概念来自于官方文档以及这两篇[1](https://www.cppstories.com/2016/02/notes-on-c-sfinae/#improved-code)[2](https://www.cppstories.com/2018/03/ifconstexpr/#c20)博客.

根据作者原文给出的例子，当我们向程序中添加了一个新的非模板函数重载时，程序便不会报错，这引出了我们之前在`instantiation`部分提到过的用于构建`overload set`的`overload Resolution`过程：

#### name lookup

根据[官方文档](https://en.cppreference.com/w/cpp/language/overload_resolution),为了编译一个函数调用，程序首先会进行`name lookup`，这一步是将程序中出现的名字与引入它的声明联系起来，比如对于语句

```c++
std::cout << std::endl;
```

编译器会进行对名称`std`的`unqualified name`查找，在头文件`<iostream>`中找到它；之后再对`cout, endl`做`qualified name`的查找，即在命名空间`std`中进行的查找过程。
...
需要注意的是，如果作用域解析符`::`左侧没有东西，那么程序会默认在`global namespace scope`中进行查找：
